...
public class ParameterMetadataProvider {
  ...
  /**
   * Builds a new {@link ParameterMetadataProvider.ParameterMetadata} for given {@link Part} and the next {@link Parameter}.
   */
  ...
  public <T> ParameterMetadataProvider.ParameterMetadata<T> next(Part part) {
    ...
    return (ParameterMetadataProvider.ParameterMetadata<T>) next(part, parameter.getType(), parameter);
  }

  /**
   * Builds a new {@link ParameterMetadataProvider.ParameterMetadata} of the given {@link Part} and type. Forwards the underlying
   * {@link Parameters} as well.
   *
   * @param <T> is the type parameter of the returned {@link ParameterMetadataProvider.ParameterMetadata}.
  ...
  public <T> ParameterMetadataProvider.ParameterMetadata<? extends T> next(Part part, Class<T> type) {
    ...
    return (ParameterMetadataProvider.ParameterMetadata<? extends T>) next(part, typeToUse, parameter);
  }
   ...
   * @param type must not be {@literal null}.
   * @param parameter providing the name for the returned {@link ParameterMetadataProvider.ParameterMetadata}.
  ...
  private <T> ParameterMetadataProvider.ParameterMetadata<T> next(Part part, Class<T> type, Parameter parameter) {
    ...
    Object value = bindableParameterValues == null ? ParameterMetadataProvider.ParameterMetadata.PLACEHOLDER : bindableParameterValues.next();

    ParameterMetadataProvider.ParameterMetadata<T> metadata = new ParameterMetadataProvider.ParameterMetadata<>(expression, part, value, escape);
