...
public class ReactiveJpaQueryMethod extends QueryMethod {
  ...
  private static final StoredProcedureAttributeSource STORED_PROCEDURE_ATTRIBUTE_SOURCE = StoredProcedureAttributeSource.INSTANCE;
   ...
   */
  protected ReactiveJpaQueryMethod(Method method, RepositoryMetadata metadata, ProjectionFactory factory,
    ...
    this.isCollectionQuery = Lazy.of(() -> (isCollectionQueryInternal() || super.isCollectionQuery()) && !NATIVE_ARRAY_TYPES.contains(this.returnType));
  ...
  /**
   * @return return true if {@link org.springframework.data.jpa.repository.Meta} annotation is available.
   * @since 3.0
   */
  ...
  /**
   * Returns the {@link org.springframework.data.jpa.repository.Meta} annotation that is applied to the method or {@code null} if not available.
   *
   * @return
   * @since 3.0
   */
...
//  }
  ...
  StoredProcedureAttributes getProcedureAttributes() {
    ...
    if (storedProcedureAttributes == null) {
      this.storedProcedureAttributes = STORED_PROCEDURE_ATTRIBUTE_SOURCE.createFrom(method, getEntityInformation());
    }
