...
public class AbstractStringBasedReactiveJpaQuery extends AbstractReactiveJpaQuery {
  ...
  public AbstractStringBasedReactiveJpaQuery(ReactiveJpaQueryMethod method, Mutiny.SessionFactory sessionFactory, String queryString,
  ...
  public Mutiny.AbstractQuery doCreateQuery(ReactiveJpaParametersParameterAccessor accessor, ReactiveJpaQueryMethod method) {
    ...
    Mutiny.AbstractQuery query = createReactiveJpaQuery(sortedQueryString, method, accessor.getSession(), accessor.getSort(), accessor.getPageable(),
  ...
  protected Mutiny.AbstractQuery doCreateCountQuery(ReactiveJpaParametersParameterAccessor accessor) {
    ...
    Mutiny.AbstractQuery query = getQueryMethod().isNativeQuery() //
   ...
   * type.
   */
  protected Mutiny.AbstractQuery createReactiveJpaQuery(String queryString, ReactiveJpaQueryMethod method, Mutiny.Session session, Sort sort, @Nullable Pageable pageable,
                                                        ReturnedType returnedType) {
    if(method.isModifyingQuery()) {
      return createReactiveJpaQueryForModifying(queryString, method, session, sort, pageable, returnedType);
    }
  ...
  private Mutiny.MutationQuery createReactiveJpaQueryForModifying(String queryString, ReactiveJpaQueryMethod method, Mutiny.Session session, Sort sort, @Nullable Pageable pageable,
                                                                  ReturnedType returnedType) {
