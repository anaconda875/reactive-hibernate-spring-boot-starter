...
public class StringQuery implements DeclaredQuery {
  ...
  StringQuery(String query, boolean isNative) {
    ...
    this.query = StringQuery.ParameterBindingParser.INSTANCE.parseParameterBindingsOfQueryIntoBindingsAndReturnCleanedQuery(query,
  ...
  enum ParameterBindingParser {
    ...
    static {
      ...
      for (StringQuery.ParameterBindingParser.ParameterBindingType type : StringQuery.ParameterBindingParser.ParameterBindingType.values()) {
    ...
    private String parseParameterBindingsOfQueryIntoBindingsAndReturnCleanedQuery(String query,
                                                                                  List<ParameterBinding> bindings, StringQuery.Metadata queryMeta) {
      ...
      StringQuery.ParameterBindings parameterBindings = new StringQuery.ParameterBindings(bindings, it -> checkAndRegister(it, bindings),
      ...
      while (matcher.find()) {
        ...
        switch (StringQuery.ParameterBindingParser.ParameterBindingType.of(typeSource)) {
          ...
          case LIKE:
            ...
            bindingFactory = (identifier) -> new ParameterBinding.LikeParameterBinding(identifier, origin, likeType);
            break;

          case IN:
            bindingFactory = (identifier) -> new ParameterBinding.InParameterBinding(identifier, origin);
            break;
   ...
   * bound to potentially unique query parameters for {@link ParameterBinding.LikeParameterBinding#prepare(Object) LIKE rewrite}.
   *
   * @author Mark Paluch
   * @since 3.1.2
   */
  static class ParameterBindings {
    ...
    ParameterBinding.BindingIdentifier register(ParameterBinding.BindingIdentifier identifier, ParameterBinding.ParameterOrigin origin,
                                                                                              Function<ParameterBinding.BindingIdentifier, ParameterBinding> bindingFactory) {
