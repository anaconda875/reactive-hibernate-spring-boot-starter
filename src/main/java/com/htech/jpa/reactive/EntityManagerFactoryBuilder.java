...
/**
 * Convenient builder for JPA EntityManagerFactory instances. Collects common configuration when
 * constructed and then allows you to create one or more {@link
 * LocalContainerEntityManagerFactoryBean} through a fluent builder pattern. The most common options
 * are covered in the builder, but you can always manipulate the product of the builder if you need
 * more control, before returning it from a {@code @Bean} definition.
 *
 * @author Dave Syer
 * @author Phillip Webb
...
public class EntityManagerFactoryBuilder {
   ...
   * @param persistenceUnitManager optional source of persistence unit information (can be null)
   */
   ...
   * @param jpaProperties the JPA properties to be passed to the persistence provider
   * @param persistenceUnitManager optional source of persistence unit information (can be null)
   * @param persistenceUnitRootLocation the persistence unit root location to use as a fallback (can
   *     be null)
   * @since 1.4.1
   */
  public EntityManagerFactoryBuilder(
  ...
  /**
   * Configure the bootstrap executor to be used by the {@link
  ...
  /**
   * Set the {@linkplain PersistenceUnitPostProcessor persistence unit post processors} to be
   * applied to the PersistenceUnitInfo used for creating the {@link
   * LocalContainerEntityManagerFactoryBean}.
   *
  ...
  public class Builder {
    ...
    /**
     * The persistence managed types, providing both the managed entities and packages the entity
    ...
    /**
     * The name of the persistence unit. If only building one EntityManagerFactory you can omit
     * this, but if there are more than one in the same application you should give them distinct
     * names.
     *
    ...
    /**
     * Generic properties for standard JPA or vendor-specific configuration. These properties
     * override any values provided in the constructor.
     *
    ...
    /**
     * The mapping resources (equivalent to {@code <mapping-file>} entries in {@code
    ...
    /**
     * Configure if using a JTA {@link DataSource}, i.e. if {@link
     * LocalContainerEntityManagerFactoryBean#setDataSource(DataSource) setDataSource} or {@link
     * LocalContainerEntityManagerFactoryBean#setJtaDataSource(DataSource) setJtaDataSource} should
     * be called on the {@link LocalContainerEntityManagerFactoryBean}.
     *
